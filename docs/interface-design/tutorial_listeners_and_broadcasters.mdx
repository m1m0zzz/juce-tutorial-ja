...
について[Button::Listener](https://docs.juce.com/master/classButton_1_1Listener.html "ボタンがクリックされたときにコールバックを受け取るために使用します。")クラスのインスタンスを含む、さまざまなボタンタイプをリッスンするために使用できます。[TextButton](https://docs.juce.com/master/classTextButton.html "標準的な菱形の背景にテキストを並べたボタン。")クラスは次のようになる。
...
では`MainContentComponent::buttonClicked()`関数に渡される。ここでは、変更をブロードキャストしたオブジェクトへのポインタが渡される。そして、このポインターを他のオブジェクトと比較することで、どのオブジェクトが変更されたかを判断できる：
...
このチュートリアルで示したようなリスナーやブロードキャスターのパラダイムを使う代わりに、最新のC++標準のラムダ関数を使うことで、ボタンのコールバックを単純化できます。これは、複雑な実装を必要としない単純なコールバックには特に有効です。
...
最後に、コールバック関数の名前をcheckTime()に変更します。\[9\]へのポインタをチェックするif() ステートメントを削除する。[Button](https://docs.juce.com/master/classButton.html "ボタンの基本クラス。")オブジェクトをチェックする必要がないからだ。[Button](https://docs.juce.com/master/classButton.html "ボタンの基本クラス。")はもうその関数を呼び出さない：
