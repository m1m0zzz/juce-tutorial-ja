...
2次元レイアウトシステムとして、[Grid](https://docs.juce.com/master/classGrid.html "グリッドレイアウト（固定列と固定行）のジオメトリを扱うコンテナ。")の両方で動作する。*行*軸と*列*軸である。同様に[FlexBox](https://docs.juce.com/master/classFlexBox.html "FlexItemオブジェクトのセットを含み、レイアウトを管理するFlexBoxコンテナを表します。")以下のプロパティは、レイアウトに以下のような影響を与える：
...
- [Span](https://docs.juce.com/master/classSpan.html "Array（配列）、vector（ベクトル）、その他同様のコンテナに格納された連続するオブジェクトに対する非所有ビュー。")は、複数のグリッドセルを埋めるために項目を拡張できます。
...
- オリエンテーションの変更に対応し、それに応じてインタフェースを変更。
