...
実際、このような[Path](https://docs.juce.com/master/classPath.html "パスとは、線と曲線が連続したもので、閉じた形を形成することもあれば、開放的な形を形成することもある。")オブジェクトの各コーナーを定義する4点を持つ矩形を指定できる：
...
この場合[Point\<float\>](https://docs.juce.com/master/classPoint.html)クラスは[Path](https://docs.juce.com/master/classPath.html "パスとは、線と曲線が連続したもので、閉じた形を形成することもあれば、開放的な形を形成することもある。")クラスを使う必要がある。しかし、Point\<int\>::toFloat()関数とPoint\<float\>::toInt()関数を使えば、2種類のポインできる。できる。
...
今すぐアプリケーションを実行してください。マウスのクリックに反応して再描画するコードを残しておけば、コンポーネントをクリックすることで新しい線のセットを生成できます。
