...
について[ValueTree](https://docs.juce.com/master/classValueTree.html "自由形式のデータを保持するために使用できる強力なツリー構造で、独自のアンドゥを扱うことができる。")クラスは究極のコンテナ・クラスであり、あらゆる種類の情報を保持できる。これが型の明らかな役割かもしれないが、型には他にも多くの側面があり、単なる構造的な役割を超えている。

#### データ保管

A[ValueTree](https://docs.juce.com/master/classValueTree.html "自由形式のデータを保持するために使用できる強力なツリー構造で、独自のアンドゥを扱うことができる。")オブジェクトは柔軟な多目的データオブジェクトです。基本的な*タイプ*という名前を持ち、名前付きプロパティの任意のセットを保持できる。これは一種の*ユニバーサル構造体*どのような（あるいはどの程度の）データを保持するかを定義する必要はない。
...
プロパティや子プロパティを操作するためのシンプルで汎用的なインタフェースを持つ。汎用的であることで、コンテンツの種類や構成に関係なく、コンテンツにアクセスするための単一のインタフェースを持つことができます。普遍的な[var](https://docs.juce.com/master/classvar.html "プリミティブ値の範囲を保持するために使用できるバリアントクラス。")クラスを共通のプロパティ・タイプとすることで、さまざまな入力を取ることができる。そのため[Identifier](https://docs.juce.com/master/classIdentifier.html "名前によってプロパティにアクセスするために設計された文字列識別子を表します。")クラスを使って、これらのプロパティを名前別に保存したり取得したりすることで、直感的にデータを整理できます。
...
においてである。[ValueTree](https://docs.juce.com/master/classValueTree.html "自由形式のデータを保持するために使用できる強力なツリー構造で、独自のアンドゥを扱うことができる。")オブジェクトとして保持されます。[var](https://docs.juce.com/master/classvar.html "プリミティブ値の範囲を保持するために使用できるバリアントクラス。")オブジェクトにアクセスできます。この多目的クラスをプロパティタイプとして選択することで、どのようなものであれ、それらにアクセスするための関数を1つにまとめることができます。のために1つの関数を使う必要はない。`int`このような関数はすべて、以下のように統一できる。[var](https://docs.juce.com/master/classvar.html "プリミティブ値の範囲を保持するために使用できるバリアントクラス。")オブジェクトがある。
...
- 限られた文字セットを強制できる：第一に、このクラスは有効なキーを構成する文字の制限を強制します。`_-:#$%`.これは少しくだらなく聞こえるかもしれないが、同じ制限（例えばXMLフォーマットやスクリプト）を持つ他のシステムとの互換性を確保することが可能になる。
...
を初期化するのが良い方法です。[Identifier](https://docs.juce.com/master/classIdentifier.html "名前によってプロパティにアクセスするために設計された文字列識別子を表します。")インスタンスに変換されます。コードから、リテラル文字列の代わりにこれらのインスタンスを使用できます。グローバルにアクセス可能な名前空間に置いたり、ファイルの静的インスタンスを使ったり、静的クラス・メンバーを使って整理することもできます。
...
これはヌル・ポインタに似ているが（実際、内部的にはまさにそうである）、固有の危険性はない。有効なノードが割り当てられるまでは、何も参照しない。ここでの主な違いは、（操作されるものが何もないため）ほとんど何の効果もないとはいえ、インタフェース関数をまだ使用できることである。

無効なノードでアクセス関数を "偶然 "呼び出すことは安全だが、それはあまり得策ではない！

### ノードが無効かどうかをチェックする方法

あなたのコードでは、メンバー関数[ValueTree::isValid()](https://docs.juce.com/master/classValueTree.html#a9c1506afbe6b840e4ab43fabf275845a "このツリーが有効なデータを参照している場合に真を返します。")を確認する。[ValueTree](https://docs.juce.com/master/classValueTree.html "自由形式のデータを保持するために使用できる強力なツリー構造で、独自のアンドゥを扱うことができる。")は空である：
...
そして、通知を受けたい動作に応じて、以下のコールバック関数を実装できる：
...
上記の行はどちらも、同じ結果を`name`変数に格納できます。これらのプロパティは[var](https://docs.juce.com/master/classvar.html "プリミティブ値の範囲を保持するために使用できるバリアントクラス。")型である。したがって、以下のコードも有効である：
...
構造体を使う場合は、これらのメンバー変数を直接設定したり取得したりする機能しかない。しかし[ValueTree](https://docs.juce.com/master/classValueTree.html "自由形式のデータを保持するために使用できる強力なツリー構造で、独自のアンドゥを扱うことができる。")ノードを使用することで、(実行時に)*変数*がある。
...
ノードには多くのプロパティを付加できるが、配列のようにいくつかの子ノードを含むこともできる。
