...
のビットをテストできる。[BigInteger](https://docs.juce.com/master/classBigInteger.html "任意に大きな整数クラス。")あたかも[BigInteger](https://docs.juce.com/master/classBigInteger.html "任意に大きな整数クラス。")オブジェクトは`bool`値をテストします。この例では、幾何学的シーケンスを生成するためにオリジナルのコードを使用し、生成された各値に対してビット3をテストする：
