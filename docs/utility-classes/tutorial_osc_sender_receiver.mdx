...
OSCLogListBoxと[OSCReceiver](https://docs.juce.com/master/classOSCReceiver.html "OSCデータを受信するためのクラス。")オブジェクトをプライベート・メンバー変数として使用します。\[2\]を定義し、ポート番号を格納する一時的なint変数を定義する。\[3\]以下の通りである：
...
もし[OSCBundle](https://docs.juce.com/master/classOSCBundle.html "OSCバンドル。")を受信すると、別のコールバック関数が呼び出され、バンドルの内容をOSCLogListBoxに転送する：
