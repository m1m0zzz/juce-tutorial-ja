...
![](../../JUCE-tutorials/_images/dot_inline_dotgraph_1.svg)
...
アプリケーションはグラフィカル・ユーザー・インタフェースを表示するので、テンプレートは
[JUCEApplication](https://docs.juce.com/master/classJUCEApplication.html)
から派生したクラスが作成されたMain.cppファイルを提供します。

[JUCEApplication](https://docs.juce.com/master/classJUCEApplication.html)
クラスは、アプリケーションのスタートアップとシャットダウンの機能を提供する抽象基底クラスです。
また、Main.cppファイルはGUIが存在するアプリケーション・ウィンドウを作成します。
...
- MainComponent.cpp：[Component](https://docs.juce.com/master/classComponent.html) クラスから派生したMainComponentの実装ファイル
...
Audio Applicationプロジェクトの詳細については、
...
アプリケーションのGUIに簡単なアニメーションを作成する場合にこのプロジェクトタイプを使用します。
...
- Main.cpp：[JUCEApplication](https://docs.juce.com/master/classJUCEApplication.html) クラスから派生したもので、アプリケーションとウィンドウの初期化コードを提供します。
- MainComponent.cpp：[OpenGLAppComponent](https://docs.juce.com/master/classOpenGLAppComponent.html) クラスから派生したMainComponentのヘッダーと実装ファイルです。
...
Main.cpp：Main.cpp：Cスタイルのmain() 関数を提供し、対応するコマンドライン引数でアプリケーションを実行します。
...
このプロジェクト・タイプでは、Main.cppファイルにクラスは作成されず、
ベース・クラスも継承されませんが、`.jucer` ファイルとJUCEライブラリ・コード・フォルダが作成されます。
...
- PluginEditor.h：[AudioProcessorEditor](https://docs.juce.com/master/classAudioProcessorEditor.html) クラスから派生したPluginEditorのヘッダーファイル。
...
このプロジェクト・タイプは、DAWまたはGUIを備えたスタンドアローン・アプリケーションでホストできるプラグインを作成する場合に使用します。
...
そして、いつでもJUCEモジュールにアクセスできる利便性と共に、ファイルを自由に追加できます。
