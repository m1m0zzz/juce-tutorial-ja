...
で見たとおりだ。[Tutorial: The fast Fourier transform](../tutorial_simple_fft/)高速フーリエ変換は、ある信号の個々の周波数成分を処理するために、時間領域の信号を周波数領域に変換できる。
...
スペクトル漏洩の影響を減らすために、フーリエ変換を行う前に信号に窓関数を適用でき、窓関数の種類によって出力への影響が異なります。以下に、JUCE DSPモジュールで利用可能ないくつかの窓とその特徴を示します：
