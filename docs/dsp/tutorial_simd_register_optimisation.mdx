...
このデモ・プロジェクトでは、読み込んだオーディオ・ファイルをIIRフィルターを通して再生し、試聴時に処理・変更できます。この最適化の目的は、同じIIRフィルターにSIMD命令セットを使用することで、CPUパワーをどれだけ軽減できるかを確認することです。
...
の中で`SIMDTutorialFilter`クラスでは、まずここに示すように、フィルタのパラメータなどのメンバー変数を定義する：
...
ProcessorDuplicator内にIIRフィルター・オブジェクトを定義することで、各チャンネルのprepare()、process()、reset()関数を個別に呼び出す心配がなくなり、モノラル・プロセッサーを自動的にマルチチャンネル・プロセッサーに変換できる。また、通過フィルタのタイプ、カットオフ周波数、シャープネスQなど、フィルタのパラメータも定義します。
...
- \[14\]最後に、インターリーブされたAudioBlockからチャンネルポインタにコピーすることで、異なるチャンネルのすべてのサンプルをデインターリーブします。[HeapBlock](https://docs.juce.com/master/classHeapBlock.html "ヒープ上のデータへのポインターを保持するための、とてもシンプルなコンテナ・クラス。")で、サンプル数とチャンネル数をSIMDRegisterのサイズとして指定します。
