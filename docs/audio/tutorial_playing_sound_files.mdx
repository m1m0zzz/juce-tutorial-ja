...
これから、これらのクラスと適切なユーザー・インタフェース・クラスを組み合わせて、サウンドファイル再生アプリケーションを作ります。この時点で、様々なフェーズについて考えておくと便利です。*運輸省*\- オーディオファイルの再生オーディオファイルがロードされると、以下の4つの状態が考えられる：
...
これにより、最低限[AudioFormatManager](https://docs.juce.com/master/classAudioFormatManager.html "利用可能なオーディオフォーマットのリストを保持し、どのフォーマットを使ってファイルを開くかを決定するクラス。")オブジェクトを使ってWAVとAIFFフォーマットのリーダーを作成できます。その他のフォーマットは、プラットフォームと`juce_audio_formats`モジュールをProjucerプロジェクトに追加した：
...
これは単にメンバー関数を呼び出しているだけであることがわかる。`changeState()`.

### 州の変更

輸送状態の変化は、この1つの機能に集約される。`changeState()`.これにより、この機能のすべてのロジックを一箇所にまとめることができます。この関数は`state`メンバーは、この新しい状態にあるときに起こる必要がある他のオブジェクトへの変更をトリガする。
...
追加[Label](https://docs.juce.com/master/classLabel.html "テキスト文字列を表示するコンポーネントで、クリックするとオプションでテキストエディタになります。")オブジェクトをインタフェースに追加する。[AudioTransportSource](https://docs.juce.com/master/classAudioTransportSource.html "AudioSourceは、PositionableAudioSourceを受け取り、再生、停止、...を可能にします。")オブジェクトを使用できます。オブジェクトは[AudioTransportSource::getCurrentPosition()](https://docs.juce.com/master/classAudioTransportSource.html#a1984ac3a6eafb12d5312c3eab5615810 "次のデータブロックが読み込まれる位置を返します。")関数を使用してこのポジションを取得する。また`MainContentComponent`クラスは[Timer](https://docs.juce.com/master/classTimer.html "指定した時間間隔で仮想メソッドへのコールバックを繰り返す。")クラスで定期的な更新を行います。`timerCallback()`関数を使用してラベルを更新することもできる。さらに[RelativeTime](https://docs.juce.com/master/classRelativeTime.html "相対的な時間の尺度。")クラスを使用して、生の秒単位の時間をより便利な分、秒、ミリ秒の形式に変換できます。
...
- また、オーディオファイルの再生状態を管理する1つの方法についても調べた。
