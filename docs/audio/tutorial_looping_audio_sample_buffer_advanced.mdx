...
このチュートリアルでは、スレッドセーフな技術を使用して、[AudioSampleBuffer](#)オブジェクトに格納された音声を再生およびループする方法を説明します。また、バックグラウンドスレッドで音声データを読み込む技術も紹介します。
...
- \[6\]: `ReferenceCountedBuffer`オブジェクト内の[AudioSampleBuffer](#)オブジェクトにアクセスします。
