...
プラグインパラメーターの自動管理パラメーターの保存とアクセスが簡単になり、特に効果的なユーザーインタフェースの構築がより簡単になります。
...
また、JUCEを使ってオーディオプラグインを構築し、これをお好みのオーディオホスト（Digital Audio Workstationなど）にロードする方法も知っておく必要があります。
...
これに加えて、入力信号の位相を反転させるphase invert\* パラメータも持っています。
...
プラグインのパラメータ管理にAudioProcessorValueTreeStateクラスを使うことには、いくつかの利点があります：
...
- [ValueTree](https://docs.juce.com/master/classValueTree.html) オブジェクトは、リスナーをアタッチできます。
...
[RangedAudioParameter](https://docs.juce.com/master/classRangedAudioParameter.html) から派生した任意のパラメータを管理でき、
...
また、XML要素のタグ名を検査することで、ValueTreeが生成するXMLがプラグインに適したValueTree型であることを確認します。
...
代わりに、[AudioProcessorValueTreeState](https://docs.juce.com/master/classAudioProcessorValueTreeState.html) クラス内のアタッチメント・クラスを使用できます。
...
また、[AudioProcessorValueTreeState](https://docs.juce.com/master/classAudioProcessorValueTreeState.html) に対してaddを呼び出すことで、プログラム的にパラメータ（またはAudioProcessorParameterGroups）を追加することもできます。
以下にその例を示します：
