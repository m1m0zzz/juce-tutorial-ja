...
デモプロジェクトでは、MIDIチャンネル10にMIDIメッセージを作成するための4つのボタンを用意しています。バスドラム、スネアドラム、クローズドハイハット、オープンハイハットです。また、ボリュームコントローラーメッセージ（連続コントローラー7）を作成するスライダーもあります。インタフェースは次のスクリーンショットのとおりです。
...
右側のパネルには、生成されたMIDIメッセージのリストがタイムスタンプ（アプリケーショ ンの起動時刻からの相対時間）とともに表示されます。
...
私たちの`addMessageToList()`関数はタイムスタンプとMIDIメッセージを解析し、インタフェースのメッセージリストに表示できるようにします：
...
- \[2\](その)[MidiBuffer](https://docs.juce.com/master/classMidiBuffer.html "タイムスタンプされたMIDIイベントのシーケンスを保持する。")クラス使用*サンプル*をMIDIメッセージのタイムスタンプの単位とします。オーディオを生成するわけではありませんが、MIDIメッセージのタイムスタンプとして使うものを選ぶ必要があります。*サンプルレート*.このメンバーを使ってサンプルレートを格納する。(一般的な値なので44,100を使用する）。
- \[3\]の中でどのタイムスタンプに到達したかを追跡する必要がある。[MidiBuffer](https://docs.juce.com/master/classMidiBuffer.html "タイムスタンプされたMIDIイベントのシーケンスを保持する。").このメンバーは、このタイムスタンプをサンプルとして保存するために使用します。
...
を修正する。`setNoteNumber()`関数と[Slider::onValueChange](https://docs.juce.com/master/classSlider.html#a680d007f6a824a28a60aa05b4045e794 "このコールバックオブジェクトにラムダを代入して、スライダーの値が変更されたときにコールさせることができます。")ヘルパー・オブジェクトでこの関数を使用します。これにより、MIDIメッセージ・イベントを未来にスケジューリングできます：
...
- \[8\]:その[MidiBuffer::clear()](https://docs.juce.com/master/classMidiBuffer.html#a220d909954450942ecd87a945a82e7aa "バッファからすべてのイベントを削除する。")関数はある範囲内のタイムスタンプを持つMIDIメッセージをバッファからクリアします。この関数は、処理したばかりのメッセージを削除するために使います。
