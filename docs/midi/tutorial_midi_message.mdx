...
このチュートリアルのデモ・プロジェクトのダウンロードはこちらから：[PIP](https://docs.juce.com/master/tutorials/PIPs/MidiMessageTutorial.zip)|[ZIP](https://docs.juce.com/master/tutorials/ZIPs/MidiMessageTutorial.zip).プロジェクトを解凍し、最初のヘッダーファイルをProjucerで開く。
...
を修正する。`setNoteNumber()`関数と[Slider::onValueChange](https://docs.juce.com/master/classSlider.html#a680d007f6a824a28a60aa05b4045e794 "このコールバックオブジェクトにラムダを代入して、スライダーの値が変更されたときにコールさせることができます。")ヘルパー・オブジェクトでこの機能を利用することができます。これにより、MIDIメッセージ・イベントを未来にスケジューリングすることができます：
