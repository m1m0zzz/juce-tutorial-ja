...
の重要な特徴のひとつである。[MPE](https://support.roli.com/article/what-is-mpe/)は、特定のコントローラー・キーボードからのすべてのノートが同じMIDIチャンネルに割り当てられるのではなく、それぞれの新しいMIDIノート・イベントに独自のMIDIチャンネルが割り当てられます。これにより、コントロール・チェンジ・メッセージやピッチ・ベンド・メッセージなどによって、個々のノートを独立してコントロールできるようになります。JUCEの実装では[MPE](https://support.roli.com/article/what-is-mpe/)演奏音は[MPENote](https://docs.juce.com/master/structMPENote.html "この構造体は、演奏中のMPEノートを表します。")オブジェクト。或いは[MPENote](https://docs.juce.com/master/structMPENote.html "この構造体は、演奏中のMPEノートを表します。")オブジェクトは以下のデータをカプセル化する：
...
ノートを演奏していない状態では、ビジュアライザーが従来のMIDIキーボードレイアウトを表していることがわかります。デモアプリケーションのビジュアライザーでは、各ノートを以下のように表現しています：
...
に渡されるもう1つの重要な引数に注目してほしい。[AudioDeviceSelectorComponent](https://docs.juce.com/master/classAudioDeviceSelectorComponent.html "AudioDeviceManagerオブジェクトのオーディオ設定を変更するためのコントロールを含むコンポーネント。")コンストラクタ`showMidiInputOptions`でなければならない。`true`をクリックして、使用可能なMIDI入力を表示します。
...
について`handleIncomingMidiMessage()`は、ユーザーインタフェースのアクティブなMIDI入力のいずれかからMIDIメッセージが受信されるたびに呼び出されます：
...
[MPEValue](https://docs.juce.com/master/classMPEValue.html "このクラスは、MPE の制御次元のいずれかの単一の値を表します。")オブジェクトを使用すると、7ビットまたは14ビットのMIDI値ソースから値を簡単に作成し、これらの値を0〜1または-1〜+1の範囲の浮動小数点値として取得できます。
...
を変更する。`MainComponent::noteStopped()`関数を使って、ノートオフ速度(*リフト*)でノートのリリース速度を変更できます。リフトを速くすると、リリース時間が短くなります。
